...
def handler(self: "PrimeBDS", sender: CommandSender, args: list[str]) -> bool:
    ...
    if not args:
        sender.send_message(f"Your current nickname is: {ColorFormat.YELLOW}{player.name_tag}")
        return True

    if args[0].lower() == "remove":
        player.name_tag = player.name  # Reset nickname
        sender.send_message(f"Your nickname has been reset to your original name: {ColorFormat.YELLOW}{player.name}")
    else:
        new_nick = args[0]
        if new_nick:
            player.name_tag = new_nick  # Set new nickname
            sender.send_message(f"Your nickname was set to: {ColorFormat.YELLOW}{new_nick}")
        else:
            sender.send_error_message(f" Nickname cannot be empty.")
