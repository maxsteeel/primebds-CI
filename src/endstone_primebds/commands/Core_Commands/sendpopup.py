...
def handler(self: "PrimeBDS", sender: CommandSender, args: list[str]) -> bool:
    ...
    if args[0] == "all":
        ...
        sender.send_message(f"A popup packet was sent to all players")
    else:
        player = self.server.get_player(args[0])
        if player:
            player.send_popup(args[1])
            sender.send_message(f"A popup packet was sent to {ColorFormat.YELLOW}{player.name}")
        else:
            sender.send_message(f"A popup packet could not be sent")
